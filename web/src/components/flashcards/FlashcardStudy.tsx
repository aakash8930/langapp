...
function StudyRunner({ deck, onRecord }: { deck: FlashcardDeck; onRecord: ReturnType<typeof useFlashcardCatalog>['local']['recordSession'] }) {
  ...
  if (complete) {
    ...
    return <StudyShell><section className="flashcard-session-summary glass"><Icon name="check-circle-2" size={44} /><p className="flashcard-kicker">LOCAL SESSION COMPLETE</p><h1>{deck.title}</h1><strong className="tabular">{recalled} / {grades.length}</strong><p>Self-recalled cards in this run. This is not an FSRS retention estimate.</p><dl><div><dt>Missed</dt><dd>{counts.missed}</dd></div><div><dt>Hard</dt><dd>{counts.hard}</dd></div><div><dt>Good</dt><dd>{counts.good}</dd></div><div><dt>Easy</dt><dd>{counts.easy}</dd></div></dl><div><button type="button" className="btn btn-primary" onClick={() => setStarted(false)}>Study again</button><Link className="btn btn-secondary" to="/flashcards-statistics">View local statistics</Link>{session.state === 'signedIn' ? <Link className="btn btn-secondary" to="/review">Review due cards</Link> : null}</div></section></StudyShell>;
  }
