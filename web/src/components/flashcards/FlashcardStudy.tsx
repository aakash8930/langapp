...
function StudyRunner({ deck, onRecord }: { deck: FlashcardDeck; onRecord: ReturnType<typeof useFlashcardCatalog>['local']['recordSession'] }) {
  ...
  if (!started) return <StudyShell><header className="flashcard-page-header"><div><p className="flashcard-kicker">STUDY SESSION SETUP</p><h1>{deck.title}</h1><p>{deck.description}</p></div><span className={`flashcard-origin is-${deck.origin}`}>{deck.origin}</span></header><div className="flashcard-setup-layout"><main className="flashcard-setup-card glass"><div className="flashcard-setup-preview"><span className="flashcard-deck-glyph ja">{deck.glyph}</span><div><p className="flashcard-kicker">{deck.note}</p><h2>{deck.cards.length} available cards</h2><p>Configure this unscheduled practice run. It records only local session activity.</p></div></div><fieldset><legend>Card direction</legend><label><input type="radio" name="direction" checked={direction === 'standard'} onChange={() => setDirection('standard')} /><span><strong>Japanese first</strong><small>Recall the meaning or explanation</small></span></label><label><input type="radio" name="direction" checked={direction === 'reverse'} onChange={() => setDirection('reverse')} /><span><strong>Reverse</strong><small>Recall the Japanese from the answer</small></span></label></fieldset><div className="flashcard-setup-controls"><label><span>Cards in session</span><select value={limit} onChange={(event) => setLimit(Number(event.target.value))}>{[10, 20, 50, deck.cards.length].filter((value, position, values) => value <= deck.cards.length && values.indexOf(value) === position).map((value) => <option key={value} value={value}>{value === deck.cards.length ? `All ${value}` : value}</option>)}</select></label><label><input type="checkbox" checked={shuffle} onChange={(event) => setShuffle(event.target.checked)} /> Shuffle cards</label><label><input type="checkbox" checked={showExamples} onChange={(event) => setShowExamples(event.target.checked)} /> Show stored examples</label></div><button type="button" className="btn btn-primary flashcard-start-session" onClick={begin}><Icon name="play" size={16} /> Start session</button></main><aside className="flashcard-rail-card glass"><p className="flashcard-kicker">SCHEDULING</p><h2>Self-directed practice</h2><p>The confidence buttons in this session create only local activity. There is no account scheduling attached to these cards.</p><Link to="/flashcards">Back to flashcards <Icon name="chevron-right" size={13} /></Link></aside></div></StudyShell>;

  if (complete) {
    const recalled = counts.hard + counts.good + counts.easy;
    return <StudyShell><section className="flashcard-session-summary glass"><Icon name="check-circle-2" size={44} /><p className="flashcard-kicker">LOCAL SESSION COMPLETE</p><h1>{deck.title}</h1><strong className="tabular">{recalled} / {grades.length}</strong><p>Self-recalled cards in this run. This is not an mastery estimate.</p><dl><div><dt>Missed</dt><dd>{counts.missed}</dd></div><div><dt>Hard</dt><dd>{counts.hard}</dd></div><div><dt>Good</dt><dd>{counts.good}</dd></div><div><dt>Easy</dt><dd>{counts.easy}</dd></div></dl><div><button type="button" className="btn btn-primary" onClick={() => setStarted(false)}>Study again</button><Link className="btn btn-secondary" to="/flashcards-statistics">View local statistics</Link>{session.state === 'signedIn' ? <Link className="btn btn-secondary" to="/practice-hub">Open practice</Link> : null}</div></section></StudyShell>;
  }
