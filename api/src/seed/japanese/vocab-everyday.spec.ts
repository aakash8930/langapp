...
import { VOCAB_EVERYDAY_GROUPS, VOCAB_EVERYDAY_LESSONS } from './vocab-everyday';
...
const ALL_WORDS = Object.values(VOCAB_EVERYDAY_GROUPS).flat();

/** The lemmas already claimed by earlier units. `lemma` is the upsert key. */
const EARLIER_LEMMAS = new Set(
  [...Object.values(VOCAB_GROUPS).flat(), ...Object.values(MARKS_GROUPS).flat()].map(
    (word) => word.lemma,
  ),
);

describe('the second vocabulary unit is readable with the kana that precede it', () => {
...
describe('the second vocabulary unit is internally consistent', () => {
  ...
  it('every lesson names a group that exists, and every group is used once', () => {
    const used = VOCAB_EVERYDAY_LESSONS.flatMap((lesson) => lesson.groups);

    for (const group of used) {
      expect(Object.keys(VOCAB_EVERYDAY_GROUPS)).toContain(group);
    }
    expect(new Set(used).size).toBe(used.length);
    expect(new Set(used)).toEqual(new Set(Object.keys(VOCAB_EVERYDAY_GROUPS)));
  });

  it('orders lessons contiguously from zero', () => {
    expect(VOCAB_EVERYDAY_LESSONS.map((lesson) => lesson.order)).toEqual(
      VOCAB_EVERYDAY_LESSONS.map((_, index) => index),
    );
  });
  ...
  it('gives every lesson enough words to fill an option set', () => {
    for (const lesson of VOCAB_EVERYDAY_LESSONS) {
      const count = lesson.groups.reduce(
        (total, group) => total + (VOCAB_EVERYDAY_GROUPS[group]?.length ?? 0),
        0,
      );
  ...
  it('is the substantial unit it was meant to be — 200 words or more', () => {
    expect(ALL_WORDS.length).toBeGreaterThanOrEqual(200);
  });
});
