  ...
  return (
    <View style={{ gap: theme.spacing.xl }}>
      ...
      <View style={{ gap: theme.spacing.sm }}>
        ...
        <TodaySummary lessons={daily.lessonsDone} />
      </View>
    </View>
  );
}

/** A quiet account of completed lessons on the learner's local day. */
function TodaySummary({ lessons }: { lessons: number }) {
  const theme = useTheme();
  return (
    <Text style={[{ fontFamily: theme.families.ui, fontSize: theme.fontSize.small, color: theme.colors.inkSoft }, theme.tabularFigures]}>
      {lessons === 0 ? 'No lesson completed yet today' : `${lessons} ${lessons === 1 ? 'lesson' : 'lessons'} today`}
    </Text>
  );
}
